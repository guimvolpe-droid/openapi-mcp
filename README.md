...
Turns a REST API into MCP tools an agent can use safely. Auth, rate-limits, retries and an
...
"Connect the AI to my system" is a common integration request. Most MCP servers answer it by wrapping
the whole API with no restrictions. This one puts a policy layer between the agent and the API:
only allowlisted operations are exposed, mutating verbs are off by default, and credentials stay
server-side.
...
- **Policy gate (safe by default).** An **allowlist** of operations. Mutating verbs
  (POST/PUT/PATCH/DELETE) are disabled unless you opt in, per the [ADR](docs/adr/0001-mcp-as-a-boundary.md).
- **Auth stays out of the agent.** API key and bearer tokens are injected into the outbound HTTP request by
...
The repo bundles its own upstream, so the demo also runs with no network access. `OpenApiMcp.DemoApi`
is a zero-dependency minimal API with a deterministic seed (25 posts), userId filtering and
...
Security posture: the server binds to **127.0.0.1** only. Exposing it further is an explicit deploy
decision (tunnel or reverse proxy with TLS). Bearer auth is **fail-closed**: with
`OPENAPI_MCP_HTTP_TOKEN` unset every request answers 503, a wrong token answers 401, and the token
...
Or run the full containerized demo, synthetic upstream plus MCP server in Streamable HTTP mode
...
- **`response.maxBodyChars`**: cap on the body returned to the model (default 16000). Oversized bodies are
  cut with an explicit `[truncated: showing N of M chars]` marker. When the upstream sends
...
The load-bearing decision, treating the server as a **security boundary**, is written up in
